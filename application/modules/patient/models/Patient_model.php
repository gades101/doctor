...
class Patient_model extends CI_Model {
   ...
   //$this->db->query("update patient set display_id=$display_id where patient_id=(select min(id) from foo where id > 4"));
	...
	public function changefollowup_detail($patient_id,$followup_date){
		$data['followup_date'] = date('Y-m-d', strtotime($this->input->post('followup_date')));
        $sql = "update " . $this->db->dbprefix('followup') . " set followup_date = ? where patient_id = ?;";
        $this->db->query($sql, array($data['followup_date'], $patient_id));
	...
	function update_discount($bill_id,$discount_amount) {
		...
		if ($query->num_rows() > 0){
			...
			$data['amount'] = $discount_amount;
			$data['mrp'] = $discount_amount;
			$this->db->update('bill_detail', $data,array('bill_id ' => $bill_id));
		}else{
			$this->db->insert('bill_detail', $data);
		}
		$sql = "update " . $this->db->dbprefix('bill') . " set total_amount = total_amount - ? where bill_id = ?;";
