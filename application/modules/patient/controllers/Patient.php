...
class Patient extends CI_Controller {
    ...
    public function patient_ajax_info($patient_id,$page_num=NULL){
		...
		if ($page_num==3){
			$payments=$this->payment_model->get_curr_fees($patient_id);
    ...
    public function visit($patient_id = NULL, $appointment_id = NULL, $app_date = NULL, $hour = NULL , $min = NULL) {
		...
		if (!isset($_SESSION["user_name"]) || $_SESSION["user_name"] == '') {
            ...
            if ($this->form_validation->run() === FALSE) {
				...
				$this->patient_model->insert_followup();
            }
			$data['patient_id'] = $patient_id;
			$data['active_modules'] = $this->module_model->get_active_modules();
			$data['patient'] = $this->patient_model->get_patient_detail($patient_id);
	...
	/** Edit Visit */
    public function edit_visit($visit_id, $patient_id,$appointment_id) {
		session_start();
		//Check if user has logged in
		if (!isset($_SESSION["user_name"]) || $_SESSION["user_name"] == '') {
            redirect('login/index/');
	...
	public function print_receipt($visit_id) {
		...
		if (!isset($_SESSION["user_name"]) || $_SESSION["user_name"] == '') {
			...
			//$template = str_replace("[clinic_name]", $clinic_logo, $template);
			//Clinic Details
	...
	function followup($patient_id) {
		...
		if (!isset($_SESSION["user_name"]) || $_SESSION["user_name"] == '') {
            ...
            $data['patient_id'] = $patient_id;
			$followup_date=$this->appointment_model->get_followup_of_patient($patient_id);
            $data['followup_date'] =$followup_date;
			$followup_details=$this->patient_model->getfollowup_detail($patient_id,$followup_date);
			$uid=$followup_details['userid'];
			$data['followup_details']=$this->patient_model->getfollowup_detail($patient_id,$followup_date);
	...
	function change_followup_date($p_id) {
		...
		if (!isset($_SESSION["user_name"]) || $_SESSION["user_name"] == '') {
			...
			$followup_date=$this->appointment_model->get_followup_of_patient($p_id);

			$this->patient_model->changefollowup_detail($p_id,$followup_date);
			redirect('appointment/index/all');
        }
    }
